...
// ============================================================
// 4. ROTA DE VISUALIZAÇÃO (CORRIGIDA - ACEITA CAMINHOS COM BARRAS)
// ============================================================
app.get('/api/gallery/view/*', (req, res) => {
  // Pega o caminho completo após /view/
  const filePath = req.params[0]; // Ex: "fotos/ensaio_01.jpg" ou "ensaio_01.jpg"
...
// ============================================================
// 5. ROTA DE DOWNLOAD (COM SUPORTE A CAMINHOS COM BARRAS)
// ============================================================
app.post('/api/gallery/download', async (req, res) => {
  const { imageKeys } = req.body; // Agora imageKeys pode ter caminhos como "fotos/ensaio_01.jpg"
...
// ============================================================
// 7. ROTA PARA LISTAR ARQUIVOS DE UMA PASTA NO S3
// ============================================================
app.get('/api/gallery/list/:folder', async (req, res) => {
  ...
  // ============================================================
// ROTA PARA GERAR THUMBNAIL DO VÍDEO (FRAME)
// ============================================================
...
app.get('/api/gallery/thumbnail/:filename', async (req, res) => {
  ...
  try {
    // 1. Baixa o vídeo do S3 temporariamente
    const videoKey = `videos/${filename}`;
    const tempVideoPath = `/tmp/${filename}`;
    const tempImagePath = `/tmp/${filename.replace('.mp4', '.jpg')}`;
    ...
    // 2. Extrai um frame do vídeo usando FFmpeg
    await new Promise((resolve, reject) => {
      exec(
        `ffmpeg -i ${tempVideoPath} -ss 00:00:01 -vframes 1 -vf "scale=400:300" ${tempImagePath}`,
        (error, stdout, stderr) => {
          if (error) {
            reject(error);
          } else {
    ...
    // Se falhar, retorna um placeholder
    res.status(404).send('Thumbnail não disponível');
  }
});
